...
BEGIN
  ...
  /*
    Gate pass is unique across:
    - booking date
    - procurement centre
    - slot start time
    - token number

    This prevents two different slots from generating
    the same gate pass number.
  */
  v_gate_pass :=
    'KQ-' ||
    to_char(v_slot.slot_date, 'YYYYMMDD') ||
    '-' ||
    LEFT(REPLACE(v_slot.centre_id::text, '-', ''), 8) ||
    '-' ||
    to_char(v_slot.start_time, 'HH24MI') ||
    '-' ||
